...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    h = z*(z>0)
    #Dropout
    p=0.2 #Probability as a seed
    u1 = np.random.binomial(1.0, p, size=h.shape) / (1-p)
    #Multiplying how much neurons'll be disabled
    #h *= u1
    #h = np.maximum(z,0)
    ...
    dh = np.dot(dout, W2.T)
    #Dropout
    u1 = np.random.binomial(1, p, size=dout.shape) / p
    #dout *= u1
    dz = np.dot(dout, W2.T) * (z > 0)
    
    grads['W2'] = np.dot(h.T, dout) / N
    grads['b2'] = np.sum(dout, axis=0) / N
    grads['W1'] = np.dot(X.T, dz) / N
  ...
  def predict(self, X):
    ...
    h = z * (z>0)
    #h = np.maximum(z,0)
    op = np.dot(h, params['W2']) + params['b2']
